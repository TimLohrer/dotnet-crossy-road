...
export class GameSocket {
    ...
    constructor(url: string) {
		...
		this.connection.on(WebsocketEvent.GameJoined, async (joinedGame: Game) => {
            ...
            wsGame.set(joinedGame);
			if (joinedGame.players.length == 1) {
                this.getUser()!.id = joinedGame.hostId;
                console.log("YOU ARE HOST!");
			}
            this.getGame()?.players.forEach(p => {
		...
		this.connection.on(WebsocketEvent.NewSection, async (lanes: Lane[]) => {
		...
		this.connection.on(WebsocketEvent.UpdatePlayerPosition, (newPlayer: Player) => {
            ...
            this.getGame()?.players.forEach(p => p.user.id === newPlayer.user.id ? newPlayer : p);
			this.getRenderer()!.renderedObjects.find(obj => obj.name === newPlayer.user.id)!.position.copy(newPlayer.position.toVector3());
		});

		this.connection.on(WebsocketEvent.PlayerLeft, (playerId: string) => this.getRenderer()!.removePlayer(playerId));
    }
